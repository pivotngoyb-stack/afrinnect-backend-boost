...
import { Calendar, MapPin, Users, Globe, Search, Clock } from 'lucide-react';
...
export default function Events() {
  ...
  // Memoize filters to prevent infinite re-queries
  const filters = useMemo(() => {
    ...
    return query;
  }, [eventType, location, myProfile?.current_country, timeFilter]);
  ...
  const { 
    ...
    isLoading,
    refetch 
  } = useInfinitePagination('events', filters, {
    pageSize: 20,
    sortBy: 'start_date',
                      ...
                      <Link to={createPageUrl(`EventDetails?id=${event.id}`)}>
                        <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer">
                          ...
                          <CardContent>
                            ...
                            >
                              {isAttending ? '✓ Attending' : isFull ? 'Event Full' : 'RSVP'}
                            </Button>
                          </CardContent>
                        </Card>
                      </Link>
