...
export default function FoundingMemberStatus({ profile }) {
  ...
  return (
    <Card className="border-2 border-amber-300 bg-gradient-to-br from-amber-50 via-background to-amber-50 overflow-hidden shadow-lg">
      {/* Header Banner */}
      ...
      <CardContent className="p-6 space-y-5">
        ...
        {isTrialActive ? (
          <div className="space-y-3 bg-background rounded-xl p-4 border">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {benefits.map((benefit, idx) => (
              <div 
                key={idx}
                className="flex items-center gap-2 bg-background border rounded-lg px-3 py-2.5 shadow-sm"
              >
                <benefit.icon size={16} className={benefit.color} />
                <span className="text-sm text-muted-foreground font-medium">{benefit.text}</span>
              </div>
            ))}
          </div>
        </div>
