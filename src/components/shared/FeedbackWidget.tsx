...
export default function FeedbackWidget() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (user) {
        // Store feedback - adapt to your table structure
        console.log('Feedback submitted:', { userId: user.id, feedback });
    ...
    <>
      <div className="fixed bottom-28 right-4 z-40 md:bottom-12">
        <Button
          ...
          className="rounded-full h-12 w-12 shadow-lg bg-background hover:bg-muted text-primary border border-border"
          size="icon"
        >
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Found a bug? Have an idea? Let us know!</DialogDescription>
          </DialogHeader>
