    ...
    >
      <div className="relative">
        <div 
          ...
          style={ringStyle}
        >
          <div className="w-full h-full bg-background p-[2px] rounded-full">
            <img
              src={photo}
              alt=""
        ...
        {isOwnProfile && (
          <div className="absolute -bottom-0.5 -right-0.5 w-7 h-7 bg-primary rounded-full flex items-center justify-center border-[3px] border-background">
            <Plus size={16} className="text-primary-foreground" strokeWidth={3} />
          </div>
        )}

        {hasStory && storyCount > 1 && !isOwnProfile && (
          <div className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1.5 bg-gradient-to-r from-pink-500 to-orange-500 rounded-full flex items-center justify-center border-2 border-background">
      ...
      <span className={`text-xs font-medium w-[76px] text-center truncate ${
        hasStory && !isViewed ? 'text-foreground' : 'text-muted-foreground'
      }`}>
